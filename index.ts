...
let twist: number = 0
let palyer: { username: string, inventory: string[], attack: number, health: number, coins: number } = {
...
let monsters: { name: string, attack: number, health: number, coins: number, originalHealth: number }[] = [{
    name: "Goblin",
    attack: 5,
    health: 30,
    coins: 7,
...
async function adventure() {
    ...
    while (journey.journey == chalk.yellow.italic('Store')) {
        let items: any = await inquirer.prompt({
            name: 'item',
            type: 'list',
